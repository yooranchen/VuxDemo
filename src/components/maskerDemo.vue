<template>
  <div>
    <div style="margin: 10px;overflow: hidden;" v-for="item in list">
      <masker style="border-radius: 2px;" :opacity="0.4">
        <div class="m-img" :style="{backgroundImage: 'url(' + item.img + ')'}"></div>
        <div slot="content" class="m-title" @click="itemClick(item)">
          {{item.title}}
          <br/>
          <span class="m-time">2016-03-08</span>
        </div>
      </masker>
    </div>
  </div>
...
<script>
  import Masker from '../vux/components/masker/index.vue'
  export default{
    data(){
      return {
        list: [
